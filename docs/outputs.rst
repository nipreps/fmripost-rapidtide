...
*fMRIPost-rapidtide* outputs conform to the :abbr:`BIDS (brain imaging data structure)`
Derivatives specification (see `BIDS Derivatives`_, along with the
upcoming `BEP 011`_ and `BEP 012`_).
*fMRIPost-rapidtide* generates three broad classes of outcomes:
...
1.  **Visual QA (quality assessment) reports**:
    ...
    of processing and ensures the transparency of *fMRIPost-rapidtide* operation.
...
For each :abbr:`BOLD (blood-oxygen level dependent)` run processed with *fMRIPost-rapidtide*,
...
The confounds generated by *fMRIPost-rapidtide* are ICA component time series,
which may be labeled as "accepted" or "rejected" based on the ICA-rapidtide classification.
...
See implementation on :mod:`~fmripost_rapidtide.workflows.bold.confounds.init_bold_confs_wf`.
