...
The module has a :py:func:`~fmripost_rapidtide.config.to_filename` function to allow writing out
the settings to hard disk in *ToML* format, which looks like:

.. literalinclude:: ../fmripost_rapidtide/data/tests/config.toml
   :language: toml
   :name: fmripost_rapidtide.toml
   :caption: **Example file representation of fMRIPost-rapidtide settings**.

This config file is used to pass the settings across processes,
using the :py:func:`~fmripost_rapidtide.config.load` function.
...
CONFIG_FILENAME = 'fmripost_rapidtide.toml'
...
if not any(
    ...
    # https://github.com/nipreps/fmripost_rapidtide/pull/2080#discussion_r409118765
    from ._warnings import logging
else:
    import logging
...
if os.getenv('IS_DOCKER_8395080871'):
    ...
    if _cgroup.exists() and 'docker' in _cgroup.read_text():
        ...
        _exec_env = 'fmripost_rapidtide-docker' if _docker_ver else 'docker'
    del _cgroup
...
class environment(_Config):
    ...
    execution environment, nipype and *fMRIPost-rapidtide* versions, etc.).
...
def load(filename, skip=None, init=True):
    ...
    filename : :py:class:`os.PathLike`
        TOML file containing fMRIPost-rapidtide configuration.
    skip : dict or None
