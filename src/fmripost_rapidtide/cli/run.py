...
def main():
    ...
    if 'pdb' in config.execution.debug:
        from fmripost_rapidtide.utils.debug import setup_exceptionhook
    ...
    if not config.execution.notrack and not config.execution.debug:
        ...
        from fmripost_rapidtide.utils.telemetry import sentry_setup, setup_migas
    ...
    fmripost_rapidtide_wf = retval.get('workflow', None)
    ...
    if fmripost_rapidtide_wf and config.execution.write_graph:
        fmripost_rapidtide_wf.write_graph(graph2use='colored', format='svg', simple_form=True)

    EXITCODE = EXITCODE or (fmripost_rapidtide_wf is None) * EX_SOFTWARE
    if EXITCODE != 0:
        sys.exit(EXITCODE)

    # Generate boilerplate
    with Manager() as mgr:
        from fmripost_rapidtide.cli.workflow import build_boilerplate

        p = Process(target=build_boilerplate, args=(str(config_file), fmripost_rapidtide_wf))
        p.start()
        p.join()
    ...
    try:
        ...
        from fmripost_rapidtide.reports.core import generate_reports
...
def migas_exit() -> None:
    ...
    from fmripost_rapidtide.utils.telemetry import send_breadcrumb
...
if __name__ == '__main__':
    raise RuntimeError(
        'fmripost_rapidtide/cli/run.py should not be run directly;\n'
        'Please `pip install` fmripost_rapidtide and use the `fmripost_rapidtide` command'
    )
