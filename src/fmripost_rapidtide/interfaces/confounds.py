...
class _ICAConfoundsInputSpec(BaseInterfaceInputSpec):
    ...
    err_on_rapidtide_warn = traits.Bool(False, usedefault=True, desc='raise error if rapidtide fails')


class _ICAConfoundsOutputSpec(TraitedSpec):
    rapidtide_confounds = traits.Either(
        None, File(exists=True, desc='output confounds file extracted from Rapidtide')
    )
    mixing = File(exists=True, desc='melodic mix file with skip_vols added back')


class ICAConfounds(SimpleInterface):
    """Extract confounds from Rapidtide result directory."""

    input_spec = _ICAConfoundsInputSpec
    output_spec = _ICAConfoundsOutputSpec

    def _run_interface(self, runtime):
        rapidtide_confounds, mixing = _get_ica_confounds(
            mixing=self.inputs.mixing,
            rapidtide_features=self.inputs.rapidtide_features,
            skip_vols=self.inputs.skip_vols,
            newpath=runtime.cwd,
        )

        if self.inputs.err_on_rapidtide_warn and rapidtide_confounds is None:
            raise RuntimeError('Rapidtide failed')

        self._results['rapidtide_confounds'] = rapidtide_confounds
        self._results['mixing'] = mixing
        return runtime


def _get_ica_confounds(mixing, rapidtide_features, skip_vols, newpath=None):
    """Extract confounds from Rapidtide result directory.
    ...
    # Load input files
    rapidtide_features_df = pd.read_table(rapidtide_features)
    motion_ics = rapidtide_features_df.loc[
        rapidtide_features_df['classification'] == 'rejected'
    ].index.values
    mixing_arr = np.loadtxt(mixing, ndmin=2)

    # Prepare output paths
    mixing_out = os.path.join(newpath, 'mixing.tsv')
    rapidtide_confounds = os.path.join(newpath, 'RapidtideAggrCompRapidtideConfounds.tsv')
    ...
    pd.DataFrame(
        aggr_mixing_arr,
        columns=[f'rapidtide_motion_{x + 1:02d}' for x in motion_ics],
    ).to_csv(rapidtide_confounds, sep='\t', index=None)

    return rapidtide_confounds, mixing_out
