...
class RapidtideClassifier(SimpleInterface):
    ...
    def _run_interface(self, runtime):
        ...
        # Add MELODIC component statistics to the Rapidtide features
        component_stats = (
        ...
        features_file = os.path.abspath('rapidtide_features.tsv')
        features_df.to_csv(features_file, sep='\t', index=False)

        # Noise components
        noise_ics = clf_df[clf_df['classification'] == 'rejected'].index.values + 1
        noise_ics_file = os.path.abspath('rapidtide_noise_ics.csv')
        with open(noise_ics_file, 'w') as f:
            f.write(','.join(map(str, noise_ics)))

        self._results['rapidtide_features'] = features_file
        self._results['rapidtide_metadata'] = metric_metadata
        self._results['rapidtide_noise_ics'] = noise_ics_file
        return runtime
