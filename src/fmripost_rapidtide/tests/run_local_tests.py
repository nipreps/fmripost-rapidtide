...
def run_tests(test_regex, test_mark):
    ...
    mounted_code = '/usr/local/miniconda/lib/python3.11/site-packages/fmripost_rapidtide'
    run_str = 'docker run --rm -ti '
    run_str += f'-v {local_patch}:{mounted_code} '
    run_str += '--entrypoint pytest '
    run_str += 'nipreps/fmripost_rapidtide:unstable '
    run_str += (
        f'{mounted_code}/fmripost_rapidtide '
        f'--data_dir={mounted_code}/fmripost_rapidtide/tests/test_data '
        f'--output_dir={mounted_code}/fmripost_rapidtide/tests/pytests/out '
        f'--working_dir={mounted_code}/fmripost_rapidtide/tests/pytests/work '
    )
    if test_regex:
