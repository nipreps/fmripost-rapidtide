...
def write_derivative_description(bids_dir, deriv_dir, dataset_links=None):
    ...
    from fmripost_rapidtide import __version__

    DOWNLOAD_URL = f'https://github.com/nipreps/fmripost_rapidtide/archive/{__version__}.tar.gz'

    bids_dir = Path(bids_dir)
    deriv_dir = Path(deriv_dir)
    desc = {
        'Name': 'fMRIPost-rapidtide- Rapidtide Postprocessing Outputs',
        ...
        'HowToAcknowledge': 'Please cite fMRIPost-rapidtide when using these results.',
    }
    ...
    if 'FMRIPOST_Rapidtide__SINGULARITY_URL' in os.environ:
        desc['GeneratedBy'][0]['Container'] = {
            'Type': 'singularity',
            'URI': os.getenv('FMRIPOST_Rapidtide__SINGULARITY_URL'),
        }
